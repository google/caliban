...
flags.DEFINE_integer("timeout_sec", 24 * 60 * 60, "timeout for all builds to complete")
...
def _create_push_step(tag: str) -> BuildStep:
  return BuildStep(step_id=f"{tag}-push", args=["push", tag], dependencies=[tag])
...
def _create_image_spec(
  ...
  return ImageSpec(
    base_image=cpu_spec.base_image,
...
def _create_specs(cfg: Dict[str, Any]) -> List[Dict[str, Any]]:
  ...
  specs += [
    _create_image_spec(cfg=c, cpu_specs=cpu_specs, gpu_specs=gpu_specs) for c in images
  ]
