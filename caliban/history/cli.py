...
def _experiment_command_str(e: Experiment) -> str:
  ...
  return (
    f'{Package(*e.container_spec.spec["package"]).script_path} '
    f'{" ".join(e.args)} '
...
def _container_spec_str(cs: ContainerSpec) -> str:
  ...
  return (
    f'{cs.id}: job_mode: {cs.spec.get("job_mode", "GPU")}, '
...
def _display_jobs_hierarchy(jobs: Iterable[Job]) -> None:
  ...
  for xg in xgroups:
    ...
    for cs in container_specs:
      ...
      cs_jobs = list(filter(lambda j: (j.experiment.container_spec == cs), xg_jobs))
      experiments = sorted(
...
def _display_recent_jobs(
  ...
  with session_scope(get_sql_engine()) as session:
    recent_jobs = (
      session.query(Job).filter(Job.user == user).order_by(Job.created.desc())
    )
...
def _display_xgroup(
  ...
  with session_scope(get_sql_engine()) as session:
    xg = (
      session.query(ExperimentGroup)
      .filter(ExperimentGroup.name == xgroup)
      .filter(ExperimentGroup.user == user)
      .first()
    )
...
def stop(args: Dict[str, Any]) -> None:
  ...
  if not dry_run and not user_verify(
    "Warning: this will potentially stop many jobs, do you wish to continue?", False
  ):
    return
  ...
  with session_scope(get_sql_engine()) as session:
    running_jobs = (
      ...
      .filter(or_(Job.status == JobStatus.SUBMITTED, Job.status == JobStatus.RUNNING))
    )
    ...
    logging.info("the following jobs will be stopped:")
    ...
    if dry_run:
      logging.info(
        "to actually stop these jobs, re-run the command without " "the --dry_run flag"
      )
      return
    ...
    logging.info(
      "requested job cancellation, please be patient as it may take "
      "a short while for this status change to be reflected in the "
      "gcp dashboard or from the `caliban status` command."
    )
  ...
  for c in container_specs:
    ...
    for j in cs_jobs:
      if j.spec.platform in [Platform.CAIP, Platform.GKE]:
        assert project_id is not None, "project id must be specified for CAIP, GKE jobs"
...
def _get_resubmit_jobs(
  ...
  if len(jobs) == 0:
    logging.error("no jobs found in experiment group")
    return None
  ...
  if not all_jobs:
    jobs = list(
      filter(
        lambda x: update_job_status(x) in [JobStatus.FAILED, JobStatus.STOPPED], jobs
      )
    )

  if len(jobs) == 0:
    logging.error("no jobs found in FAILED or STOPPED state")
    return None
...
def resubmit(args: Dict[str, Any]) -> None:
  ...
  if xgroup is None:
    logging.error("you must specify an experiment group for this command")
    return
  ...
  with session_scope(get_sql_engine()) as session:
    ...
    logging.info("the following jobs would be resubmitted:")
    for j in jobs:
      logging.info(_experiment_command_str(j.experiment))
      logging.info(f"  job {_job_str(j)}")
    ...
    if dry_run:
      logging.info(
        "to actually resubmit these jobs, run this command again "
        "without the --dry_run flag"
      )
      return

    # make sure
    if not user_verify(f"do you wish to resubmit these {len(jobs)} jobs?", False):
      return
    ...
    job_specs = [
      replace_job_spec_image(spec=j.spec, image_id=image_id_map[j]) for j in jobs
    ]
    ...
    for platform in [Platform.CAIP, Platform.GKE, Platform.LOCAL]:
      ...
      try:
        ...
        logging.error("there was an error submitting some jobs")
