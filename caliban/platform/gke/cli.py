...
def _with_cluster(fn):
  ...
  def wrapper(
    args: dict, project_id: str, creds: Credentials, zone: str = k.ZONE_DEFAULT
  ):
...
# ----------------------------------------------------------------------------
def _check_for_existing_cluster(cluster_name: str, project_id: str, creds: Credentials):
  ...
  logging.info("{} clusters already exist for this project:".format(len(clusters)))
  for c in clusters:
    logging.info(c)

  return util.user_verify("Do you really want to create a new cluster?", default=False)
...
def _cluster_delete(args: dict, cluster: Cluster) -> None:
  ...
  if util.user_verify(
    "Are you sure you want to delete {}?".format(cluster.name), default=False
  ):
    cluster.delete()
...
def _node_pool_ls(args: dict, cluster: Cluster) -> None:
  ...
  for p in np:
    ...
    logging.info(
      FMT % (p.name, p.config.machine_type, accel, p.autoscaling.max_node_count)
    )
...
def _job_commands(args) -> None:
  """job commands"""
  JOB_CMDS = {"ls": _job_ls, "submit": _job_submit, "submit_file": _job_submit_file}
