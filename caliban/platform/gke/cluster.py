...
from caliban.platform.cloud.types import (
  GPU,
  TPU,
  Accelerator,
  GPUSpec,
...
def _create_cluster_spec(
  ...
  cluster_spec = {
    ...
    "releaseChannel": {"channel": release_channel.value},
...
# ----------------------------------------------------------------------------
def _cluster_create_request_body(
  project_id: str, zone: str, cluster_spec: dict
) -> dict:
...
class Cluster(object):
  ...
  # --------------------------------------------------------------------------
  def __init__(
    self, name: Optional[str], project_id: str, zone: str, credentials: Credentials
  ):
  ...
  def connect(self) -> bool:
    ...
    self.connected = self._core_api.get_api_resources(async_req=False) is not None
  ...
  def _set_gke_cluster(self) -> bool:
    ...
    cluster_list = util.get_gke_clusters(
      self._cluster_client, self.project_id, self.zone
    )
  ...
  # --------------------------------------------------------------------------
  @staticmethod
  def list(project_id: str, creds: Credentials, zone: str = "-") -> Optional[List[str]]:
  ...
  @staticmethod
  def get(
    name: Optional[str], project_id: str, zone: str, creds: Credentials
  ) -> "Optional[Cluster]":
    ...
    cluster = Cluster(name=name, project_id=project_id, zone=zone, credentials=creds)
  ...
  def container_limits(
    accelerator: Optional[Accelerator], count: int = 1, preemptible_tpu: bool = True
    ...
    logging.error("error: invalid accelerator type: {}".format(type(accelerator)))
  ...
  def template_metadata(
    accelerator: Optional[Accelerator] = None, tpu_driver: str = k.DEFAULT_TPU_DRIVER
  ...
  def node_selector(
    ...
    if isinstance(accelerator, GPU):
      selector[k.NODE_SELECTOR_GKE_ACCELERATOR] = accelerator.value.lower().replace(
        "_", "-"
      )
  ...
  def get_job(self, job_name: str) -> Optional[V1Job]:
    ...
    jobs = self._batch_api.list_job_for_all_namespaces(
      watch=False, field_selector=f"metadata.name={job_name}"
    ).items
    if len(jobs) != 1:
      return None
    return jobs[0]
  ...
  def delete_job(
    ...
    try:
      ...
      _rsp = self._batch_api.delete_namespaced_job(
        name=job_name,
        namespace=namespace,
        propagation_policy="Foreground",
      )
    except Exception as e:
      logging.error(f"error deleting job {job_name} from cluster {self.name}: {e}")
      return False
  ...
  def create_v1jobs(
    ...
    return [cls.create_v1job(job_spec=s, name=name, labels=labels) for s in job_specs]
    ...
    rsp = (
      ...
      .get(
        name="projects/{}/locations/{}/clusters/{}".format(
          self.project_id, self.zone, self.name
        )
      )
      .execute()
    )
  ...
  def validate_gpu_spec(self, gpu_spec: Optional[GPUSpec]) -> bool:
    ...
    if not util.validate_gpu_spec_against_limits(gpu_spec, gpu_limits, "cluster"):
      return False
  ...
  @connected(None)
  def apply_daemonset(
    self, daemonset: V1DaemonSet, namespace: str = k.DEFAULT_NAMESPACE
  ...
  def delete(self):
    ...
    _op = self._cluster_client.delete_cluster(
  ...
  def create_request(
    ...
    resource_limits = util.generate_resource_limits(compute_api, project_id, region)
    ...
    request_body = _cluster_create_request_body(
      ...
      _create_cluster_spec(
        cluster_name, zone, node_zones, resource_limits, release_channel
      ),
    )

    # see https://cloud.google.com/kubernetes-engine/docs/reference/rest/v1/projects.zones.clusters/create
    return (
      cluster_api.projects()
      .zones()
      .clusters()
      .create(projectId=project_id, zone=zone, body=request_body)
    )
  ...
  def create(
    ...
    rsp = util.wait_for_operation(
      cluster_api,
      "projects/{}/locations/{}/operations/{}".format(project_id, zone, operation_name),
    )
    ...
    if cluster is None:
      logging.error("error: unable to connect to cluster {}".format(cluster_name))
