...
def _run_cmd(job_mode: c.JobMode, run_args: Optional[List[str]] = None) -> List[str]:
...
def log_job_spec_instance(job_spec: JobSpec, i: int) -> JobSpec:
  ...
  """
  args = ce.experiment_to_args(job_spec.experiment.kwargs, job_spec.experiment.args)
  logging.info("")
...
def local_callback(idx: int, job: Job) -> None:
  ...
  if job.status == JobStatus.SUCCEEDED:
    ...
    args = ce.experiment_to_args(job.spec.experiment.kwargs, job.spec.experiment.args)
...
def run_experiments(
  ...
  **build_image_kwargs,
  ...
  with session_scope(engine) as session:
    ...
    try:
      execute_jobs(job_specs=job_specs, dry_run=dry_run, caliban_config=caliban_config)
