...
def test_container_spec(engine: Engine):
  ...
  with session_scope(engine) as session:
    ...
    xg = ExperimentGroup()
    Experiment.get_or_create(xgroup=xg, container_spec=s)
...
def test_experiment_group(engine: Engine):
  ...
  with session_scope(engine) as session:
    xg = (
      ...
      .filter(ExperimentGroup.created > test_timestamp)
      .all()
    )
    assert len(xg) == 1
    xg = xg[0]
...
def test_job(engine: Engine):
  ...
  with session_scope(engine) as session:
    ...
    _job = Job(spec=jspec, container="container0", details={"job_id": 123})
    session.add(e)
